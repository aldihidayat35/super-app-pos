...
@section('content')
    ...
    <x-metronic.card title="Item Invoice" class="mb-5">
        <div class="table-responsive"><table class="table align-middle"><thead><tr><th>Deskripsi</th><th>Qty</th><th>Harga</th><th>Diskon</th><th>Pajak</th><th>Total</th></tr></thead><tbody>@foreach($invoice->items as $item)<tr><td>{{ $item->description }}</td><td>{{ qty($item->quantity) }} {{ $item->unit_name_snapshot }}</td><td>{{ App\Support\CurrencyFormatter::rupiah($item->unit_price) }}</td><td>{{ App\Support\CurrencyFormatter::rupiah($item->discount_amount) }}</td><td>{{ App\Support\CurrencyFormatter::rupiah($item->tax_amount) }}</td><td class="fw-bold">{{ App\Support\CurrencyFormatter::rupiah($item->line_total) }}</td></tr>@endforeach</tbody></table></div>
    </x-metronic.card>

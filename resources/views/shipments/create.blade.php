    ...
    <x-metronic.card title="Form Shipment">
        ...
        @if($order)
            <form method="POST" action="{{ route('shipments.store') }}" class="row g-4">
                ...
                <div class="col-12"><div class="table-responsive"><table class="table"><thead><tr><th>Produk</th><th>Reserved</th><th>Qty Shipment</th></tr></thead><tbody>@foreach($order->items as $item)<tr><td>{{ $item->product_name_snapshot }}</td><td>{{ $item->reserved_quantity }}</td><td><input name="planned_quantities[{{ $item->id }}]" type="number" step="0.0001" min="0" max="{{ $item->reserved_quantity }}" value="{{ $item->reserved_quantity }}" class="form-control"></td></tr>@endforeach</tbody></table></div></div>

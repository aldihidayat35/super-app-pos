...
@section('content')
    <x-metronic.card title="{{ $employee ? 'Edit Karyawan' : 'Tambah Karyawan' }}">
        <form method="POST" action="{{ $employee ? route('attendance.employees.update', $employee) : route('attendance.employees.store') }}" class="row g-3">
            ...
            <div class="col-md-5"><x-metronic.form-group name="name" label="Nama" required><input name="name" value="{{ old('name', $employee?->name ?? $selectedUser?->name) }}" class="form-control"></x-metronic.form-group></div>
            <div class="col-md-4"><x-metronic.form-group name="user_id" label="User Opsional"><select name="user_id" class="form-select"><option value="">Tanpa user</option>@foreach($users as $user)<option value="{{ $user->id }}" @selected((int) old('user_id', $selectedUserId) === $user->id)>{{ $user->name }} — {{ $user->email }}</option>@endforeach</select></x-metronic.form-group></div>
            <div class="col-md-4"><x-metronic.form-group name="work_location_id" label="Lokasi Kerja" required><select name="work_location_id" class="form-select">@foreach($locations as $location)<option value="{{ $location->id }}" @selected((int) old('work_location_id', $selectedWorkLocationId) === $location->id)>{{ $location->name }}</option>@endforeach</select></x-metronic.form-group></div>
            ...
            <div class="col-md-3"><x-metronic.form-group name="whatsapp_number" label="Nomor WA"><input name="whatsapp_number" value="{{ old('whatsapp_number', $employee?->whatsapp_number ?? $selectedUser?->phone_number) }}" class="form-control"></x-metronic.form-group></div>

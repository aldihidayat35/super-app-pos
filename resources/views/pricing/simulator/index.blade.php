            ...
            <x-metronic.card title="Parameter Simulasi">
                <form method="GET" action="{{ route('pricing.simulator.index') }}">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6"><x-metronic.form-group name="quantity" label="Qty" required><input type="number" step="1" min="1" name="quantity" value="{{ qty_input($filters['quantity'] ?? 1) }}" class="form-control"></x-metronic.form-group></div>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-row-dashed align-middle">
                            ...
                            @forelse($result['candidates'] as $candidate)
                                <tr><td>{{ $candidate['source'] }}</td><td>Rp {{ number_format((float) $candidate['price_base'], 0, ',', '.') }}</td><td>{{ $candidate['priority'] }}</td><td>{{ $candidate['reason'] }}</td></tr>
                            @empty

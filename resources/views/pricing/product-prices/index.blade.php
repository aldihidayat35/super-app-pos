...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function updateCounter() {
            ...
            const submitButton = document.getElementById('save-product-prices');
            if (submitButton && !submitButton.dataset.permissionDisabled) submitButton.disabled = sel === 0;
        }
        ...
        document.getElementById('product-price-form')?.addEventListener('submit', function () {
            const submitButton = document.getElementById('save-product-prices');
            if (!submitButton || getSelectedValues().length === 0) return;
...
@section('content')
    ...
    <x-metronic.card title="Input Ring Harga">
        <form method="POST" action="{{ route('pricing.product-prices.store') }}" class="row g-3 align-items-end" id="product-price-form">
            @csrf
            <div class="col-md-4">
            ...
            <div class="col-md-4"><x-metronic.form-group name="branch_id" label="Cabang" help="Kosongkan untuk memberlakukan harga pada semua cabang."><select name="branch_id" class="form-select" data-control="select2" data-placeholder="Pilih cabang" data-allow-clear="true"><option value=""></option>@foreach($branches as $branch)<option value="{{ $branch->id }}" @selected((string) old('branch_id') === (string) $branch->id)>{{ $branch->code }} — {{ $branch->name }}</option>@endforeach</select></x-metronic.form-group></div>
            ...
            <div class="col-md-12"><button class="btn btn-primary" id="save-product-prices" disabled @cannot('prices.update') data-permission-disabled="true" @endcannot><span class="indicator-label"><i class="ki-outline ki-check fs-5"></i> Simpan Harga</span><span class="indicator-progress">Menyimpan... <span class="spinner-border spinner-border-sm ms-2"></span></span></button></div>
        </form>
    </x-metronic.card>
                        ...
                        <td>
                            @if($resolved['approval_required'])
                                <div class="d-flex flex-column gap-2">
                                    @foreach($resolved['approval_reasons'] as $reason)
                                        @php($warning = App\Support\PriceWarningPresenter::get($reason))
                                        <div>
                                            <span class="badge badge-light-{{ $warning['badge'] }}" data-bs-toggle="tooltip" title="{{ $warning['description'] }}"><i class="ki-outline {{ $warning['icon'] }} fs-7 me-1"></i>{{ $warning['label'] }}</span>
                                            <div class="text-muted fs-8 mt-1">{{ $warning['description'] }}</div>
                                            @if($reason === 'below_minimum')
                                                <div class="text-danger fs-8">{{ App\Support\CurrencyFormatter::rupiah(App\Support\Decimal::sub($resolved['minimum_price'], $price->recommended_price, 2)) }} di bawah batas {{ App\Support\CurrencyFormatter::rupiah($resolved['minimum_price']) }}</div>
                                            @elseif($reason === 'overpricing')
                                                <div class="text-warning fs-8">{{ App\Support\CurrencyFormatter::rupiah(App\Support\Decimal::sub($price->recommended_price, $resolved['maximum_price'], 2)) }} di atas batas {{ App\Support\CurrencyFormatter::rupiah($resolved['maximum_price']) }}</div>
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
                            @else
                                @php($warning = App\Support\PriceWarningPresenter::get('normal'))
                                <span class="badge badge-light-success" data-bs-toggle="tooltip" title="{{ $warning['description'] }}"><i class="ki-outline {{ $warning['icon'] }} fs-7 me-1"></i>{{ $warning['label'] }}</span>
                            @endif
                        </td>
                        <td class="text-end">
                            @can('update', $price)
                                <a href="{{ route('pricing.product-prices.edit', $price) }}" class="btn btn-sm btn-light-primary" title="Kelola harga">
                                    <i class="ki-outline ki-pencil fs-5"></i> Kelola
                                </a>
                            @endcan
                            @can('delete', $price)

        ...
        <x-metronic.card title="Produk Sering Dibeli">
            <div class="table-responsive"><table class="table align-middle"><thead><tr><th>Produk</th><th>Pernah Dibeli</th><th>Terakhir</th><th class="w-175px">Qty Baru</th></tr></thead><tbody>
                @forelse($items as $index => $item)
                    <tr><td><input type="hidden" name="items[{{ $index }}][product_id]" value="{{ $item->product_id }}"><input type="hidden" name="items[{{ $index }}][unit_id]" value="{{ $item->base_unit_id }}"><div class="fw-bold">{{ $item->name }}</div><div class="text-muted">{{ $item->sku }}</div></td><td>{{ $item->order_count }}x · {{ $item->total_quantity }}</td><td>{{ $item->last_ordered_at ? \Illuminate\Support\Carbon::parse($item->last_ordered_at)->format('d/m/Y') : '-' }}</td><td><input type="number" step="0.0001" min="0.0001" name="items[{{ $index }}][quantity]" value="1" class="form-control"></td></tr>
                @empty

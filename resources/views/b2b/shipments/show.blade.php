...
@section('content')
    <x-metronic.page-title :title="$shipment->number" :description="$shipment->number . ' — Status, timeline, dan bukti pengiriman.'">
    ...
    <x-metronic.card title="Item"><div class="table-responsive"><table class="table"><thead><tr><th>Produk</th><th>Dikirim</th><th>Diterima</th><th>Status</th></tr></thead><tbody>@foreach($shipment->items as $item)<tr><td>{{ $item->orderItem?->product_name_snapshot }}</td><td>{{ qty($item->quantity_shipped) }}</td><td>{{ qty($item->quantity_delivered) }}</td><td>{{ $item->status }}</td></tr>@endforeach</tbody></table></div></x-metronic.card>
@endsection

        ...
        <form method="POST" action="{{ route('retail.stock-transfers.receive', $transfer) }}" enctype="multipart/form-data">
            ...
            <div class="table-responsive">
                <table class="table table-row-dashed align-middle">
                    ...
                    <tbody>@foreach($transfer->items as $item)<tr><td>{{ $item->product_sku_snapshot }}<div class="text-muted">{{ $item->product_name_snapshot }}</div></td><td>{{ $item->quantity_shipped }}</td><td>{{ $item->quantity_received }}</td><td class="fw-bold">{{ $item->inTransitQuantity() }}</td><td><input name="items[{{ $item->id }}][quantity_received]" type="number" min="0" step="0.0001" value="{{ $item->inTransitQuantity() }}" class="form-control form-control-sm"></td><td><input name="items[{{ $item->id }}][quantity_damaged]" type="number" min="0" step="0.0001" value="0" class="form-control form-control-sm"></td><td><input name="items[{{ $item->id }}][quantity_discrepancy]" type="number" min="0" step="0.0001" value="0" class="form-control form-control-sm"></td><td><input name="items[{{ $item->id }}][notes]" class="form-control form-control-sm"></td></tr>@endforeach</tbody>
                </table>
            </div>

...
@section('content')
    ...
    <div class="row g-6">
        ...
        <div class="col-lg-8"><x-metronic.card title="Item"><table class="table"><thead><tr><th>Produk</th><th>Qty</th><th>Good</th><th>Rusak</th><th>Reject</th><th>Nilai</th></tr></thead><tbody>@foreach($return->items as $item)<tr><td>{{ $item->product_sku_snapshot }} — {{ $item->product_name_snapshot }}</td><td>{{ $item->quantity_requested }}</td><td>{{ $item->quantity_accepted_good }}</td><td>{{ $item->quantity_accepted_damaged }}</td><td>{{ $item->quantity_rejected }}</td><td>{{ \App\Support\CurrencyFormatter::rupiah($item->line_value) }}</td></tr>@endforeach</tbody></table></x-metronic.card></div>
    </div>
    <x-metronic.card title="Mutasi & Settlement" class="mt-6"><div class="row"><div class="col-md-6"><h6>Mutasi Stok</h6><ul>@forelse($return->stockMutations as $mutation)<li>{{ $mutation->mutation_type->label() }} {{ $mutation->quantity_on_hand_change }} — {{ $mutation->product?->name }}</li>@empty<li>Belum ada mutasi.</li>@endforelse</ul></div><div class="col-md-6"><h6>Settlement</h6><ul>@forelse($return->settlements as $settlement)<li>{{ $settlement->resolution->label() }} — {{ \App\Support\CurrencyFormatter::rupiah($settlement->amount) }} — {{ $settlement->document_no }}</li>@empty<li>Belum settlement.</li>@endforelse</ul></div></div></x-metronic.card>
@endsection

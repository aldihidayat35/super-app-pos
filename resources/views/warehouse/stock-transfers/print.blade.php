...
<body onload="window.print()">
    ...
    <table><thead><tr><th>Produk</th><th>Approved</th><th>Picked</th><th>Shipped</th><th>Catatan</th></tr></thead><tbody>@foreach($transfer->items as $item)<tr><td>{{ $item->product_sku_snapshot }} - {{ $item->product_name_snapshot }}</td><td>{{ qty($item->quantity_approved) }}</td><td>{{ qty($item->quantity_picked) }}</td><td>{{ qty($item->quantity_shipped) }}</td><td>{{ $item->notes ?: '-' }}</td></tr>@endforeach</tbody></table>

...
@section('content')
    <x-metronic.card>
        <form method="GET" class="row g-3 mb-6">
            <div class="col-md-3"><input name="q" value="{{ $filters['q'] ?? '' }}" class="form-control form-control-solid" placeholder="Nomor transfer"></div>
            <div class="col-md-3"><input name="source_reference" value="{{ $filters['source_reference'] ?? '' }}" class="form-control form-control-solid" placeholder="Nomor permintaan restok"></div>
            ...
            <div class="col-md-3"><input type="date" name="date_from" value="{{ $filters['date_from'] ?? '' }}" class="form-control form-control-solid" aria-label="Tanggal mulai"></div>
            <div class="col-md-3"><input type="date" name="date_to" value="{{ $filters['date_to'] ?? '' }}" class="form-control form-control-solid" aria-label="Tanggal selesai"></div>
            <div class="col-md-2"><button class="btn btn-light-primary w-100">Filter</button></div>
        </form>
        <div class="table-responsive">
            <table class="table table-row-dashed align-middle">
                <thead><tr class="text-muted fw-bold text-uppercase fs-7"><th>Nomor Transfer</th><th>Tanggal</th><th>Lokasi Asal</th><th>Lokasi Tujuan</th><th>Item / Qty</th><th>Sumber Permintaan</th><th>Status</th><th>Dibuat Oleh</th><th>Aksi</th></tr></thead>
                <tbody>
                @forelse($transfers as $transfer)
                    <tr>
                        <td class="fw-bold">{{ $transfer->number }}</td><td>{{ $transfer->transfer_date?->format('d/m/Y') }}</td><td>{{ $transfer->sourceWorkLocation?->name }}</td><td>{{ $transfer->destinationWorkLocation?->name }}</td>
                        <td>{{ $transfer->items->count() }} item / {{ qty($transfer->items->reduce(fn($carry, $item) => \App\Support\Decimal::add($carry, (string) $item->quantity_requested), '0.0000')) }}</td>
                        <td>@if($transfer->restockRequest)<a href="{{ route('retail.restock-requests.show', $transfer->restockRequest) }}" class="fw-bold">{{ $transfer->restockRequest->number }}</a>@else<span class="text-muted">Manual</span>@endif</td>
                ...
                @empty
                    <tr><td colspan="9"><x-metronic.empty-state title="Belum ada transfer stok" description="Transfer dari gudang ke toko/antar lokasi akan tampil di sini." /></td></tr>
                @endforelse
                </tbody>
            </table>
        </div>

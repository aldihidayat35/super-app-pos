                    ...
                    @foreach($selectedPo->items as $index => $item)
                        ...
                        <tr>
                            <td>
                                ...
                                <div class="text-muted">{{ $item->unit_name_snapshot }} x {{ $item->conversion_factor_snapshot }}</div>
                            </td>
                            <td>{{ $item->quantity_ordered }} / {{ $item->quantity_received }} / <span class="fw-bold">{{ $outstanding }}</span></td>
                            <td><input type="number" step="0.0001" min="0" name="items[{{ $index }}][quantity_received]" value="{{ old("items.$index.quantity_received", $draftItem?->quantity_received ?? $outstanding) }}" class="form-control form-control-sm"></td>
                            <td><input type="number" step="0.0001" min="0" name="items[{{ $index }}][quantity_accepted]" value="{{ old("items.$index.quantity_accepted", $draftItem?->quantity_accepted ?? $outstanding) }}" class="form-control form-control-sm"></td>
                            <td><input type="number" step="0.0001" min="0" name="items[{{ $index }}][quantity_rejected]" value="{{ old("items.$index.quantity_rejected", $draftItem?->quantity_rejected ?? 0) }}" class="form-control form-control-sm"></td>
                            <td><input type="number" step="0.0001" min="0" name="items[{{ $index }}][quantity_damaged]" value="{{ old("items.$index.quantity_damaged", $draftItem?->quantity_damaged ?? 0) }}" class="form-control form-control-sm"></td>
                            <td><input type="number" step="0.0001" min="0" name="items[{{ $index }}][quantity_returned_to_supplier]" value="{{ old("items.$index.quantity_returned_to_supplier", $draftItem?->quantity_returned_to_supplier ?? 0) }}" class="form-control form-control-sm"></td>

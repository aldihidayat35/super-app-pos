...
@section('content')
    ...
    <x-metronic.card class="mb-5">
        <form method="GET" class="row g-4 align-items-end">
            ...
            <div class="col-xl-2 col-md-4"><label class="form-label fw-semibold">Verifikasi</label><select name="verification_status" class="form-select form-select-solid"><option value="">Semua verifikasi</option>@foreach($verificationStatuses as $verificationStatus)<option value="{{ $verificationStatus->value }}" @selected(($filters['verification_status'] ?? '') === $verificationStatus->value)>{{ $verificationStatus->label() }}</option>@endforeach</select></div>
    ...
    @php($attendanceKpis = [
        ...
        ['Menunggu Verifikasi', number_format($metrics['pending_verification'], 0, ',', '.'), '', 'ki-shield-tick', 'warning', 'Jam pulang perlu diperiksa kepala lokasi'],
        ['Ditolak', number_format($metrics['rejected_verification'], 0, ',', '.'), '', 'ki-cross-circle', 'danger', 'Perlu koreksi dan verifikasi ulang'],
        ['Pulang Darurat', number_format($metrics['supervisor_check_outs'], 0, ',', '.'), '', 'ki-information-5', 'info', 'Dicatat kepala lokasi dengan alasan'],
    ...
    <x-metronic.card title="Detail Kehadiran">
        <div class="table-responsive"><table class="table table-row-dashed align-middle"><thead><tr class="text-muted text-uppercase fs-8 fw-bold"><th>Tanggal</th><th>Karyawan</th><th>Lokasi</th><th>Masuk</th><th>Pulang</th><th>Status</th><th>Verifikasi</th><th>Deviasi</th><th>Jam Kerja</th></tr></thead><tbody>
            @forelse($attendances as $attendance)<tr><td>{{ $attendance->attendance_date?->format('d/m/Y') }}</td><td><div class="fw-semibold text-gray-900">{{ $attendance->employee?->name }}</div></td><td>{{ $attendance->workLocation?->name }}</td><td>{{ $attendance->check_in_at?->format('H:i') ?: '—' }}</td><td><div>{{ $attendance->check_out_at?->format('H:i') ?: '—' }}</div>@if($attendance->check_out_method === 'supervisor')<span class="badge badge-light-warning mt-1">Pulang darurat</span>@endif</td><td><x-metronic.status-badge :status="$attendance->status->value" :label="$attendance->status->label()" /></td><td><span @class(['badge', 'badge-light-success' => $attendance->verification_status->value === 'approved', 'badge-light-danger' => $attendance->verification_status->value === 'rejected', 'badge-light-warning' => ! in_array($attendance->verification_status->value, ['approved', 'rejected'], true)])>{{ $attendance->verification_status->label() }}</span>@if($attendance->verification_note)<div class="text-muted fs-8 mt-1">{{ $attendance->verification_note }}</div>@endif</td><td><div>{{ $attendance->late_minutes }} menit telat</div><div class="text-muted fs-8">{{ $attendance->early_leave_minutes }} menit pulang cepat</div></td><td>{{ intdiv((int) $attendance->worked_minutes, 60) }}j {{ (int) $attendance->worked_minutes % 60 }}m</td></tr>@empty<tr><td colspan="9"><x-metronic.empty-state title="Belum ada data kehadiran" description="Ubah filter atau tunggu absensi tercatat." /></td></tr>@endforelse

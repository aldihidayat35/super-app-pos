...
@section('content')
    ...
    <x-metronic.card title="Filter Audit">
        <form method="GET" class="row g-3 mb-5">@if(request()->integer('actor_user_id') > 0)<input type="hidden" name="actor_user_id" value="{{ request()->integer('actor_user_id') }}">@endif<div class="col-md-2"><input name="module" value="{{ request('module') }}" class="form-control" placeholder="Module"></div><div class="col-md-3"><input name="event" value="{{ request('event') }}" class="form-control" placeholder="Event/action"></div><div class="col-md-2"><input type="date" name="from" value="{{ request('from') }}" class="form-control"></div><div class="col-md-2"><input type="date" name="to" value="{{ request('to') }}" class="form-control"></div><div class="col-md-2"><button class="btn btn-light-primary w-100">Filter</button></div></form>
        @if(request()->integer('actor_user_id') > 0)<div class="alert alert-light-primary py-3">Menampilkan audit untuk pengguna terpilih. <a href="{{ route('audit-logs.index') }}" class="fw-bold">Hapus filter</a></div>@endif

...
class PosWorkflowTest extends TestCase
{
    ...
    public function test_p16_pages_can_be_opened_and_barcode_search_works(): void
    {
        ...
        $this->actingAs($this->cashier)->get(route('retail.pos.index'))
            ...
            ->assertSee('class="pos-page-root"', false)
            ->assertSee('app-default pos-page', false)
            ->assertSee('data-kt-app-sidebar-collapse=on', false)
            ->assertSee('aria-expanded="false"', false)
            ->assertSee('ki-arrow-right', false)
            ->assertSee('modal-xl', false)
            ->assertSee('pos-cart-main', false)
            ->assertSee('pos-cart-details', false)
            ->assertSee('const qtyInput', false)
            ->assertSee('body.pos-page #kt_app_toolbar', false)
            ->assertSeeInOrder(['clearTransaction();', 'successModal.show();'], false)
            ->assertSee('BAYAR');
